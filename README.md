<<<<<<< HEAD
# VPN Bot - Сервис продажи VPN-подписок

Полнофункциональный сервис для продажи VPN-подписок через 3X-UI с веб-панелью администратора, Telegram-ботом и автоматической установкой.

## 🚀 Возможности

### Веб-панель администратора
- 🔐 Авторизация с JWT
- 📊 Дашборд с статистикой (выручка, пользователи, платежи)
- 👥 Управление пользователями (просмотр, продление, отключение, удаление)
- 💸 История платежей с фильтрами
- 💳 Настройки тарифов
- ⚙️ Настройки интеграций (3X-UI, Юкасса, Telegram)
- 📦 Логи всех операций
- 🔧 DevOps функции (перезапуск сервисов, обновление)

### Telegram-бот
- 🛒 Покупка VPN-подписок
- 🔄 Продление подписки
- 📅 Проверка статуса
- 🔗 Получение конфигураций
- ⚙️ Личный кабинет
- 💰 Оплата через Юкассу

### Интеграции
- 🔗 3X-UI API для управления клиентами
- 💳 Юкасса для платежей
- 🤖 Telegram Bot API

## 🛠 Технологии

- **Backend**: Python FastAPI, SQLite, SQLAlchemy
- **Frontend**: Next.js 14, React, Tailwind CSS, shadcn/ui
- **Bot**: Python aiogram 3
- **Инфраструктура**: Docker, docker-compose, nginx, certbot

## 📦 Установка

### Автоматическая установка (рекомендуется)

```bash
# Скачайте репозиторий
git clone https://github.com/your-repo/vpn-bot.git
cd vpn-bot

# Запустите скрипт установки
chmod +x install.sh
sudo ./install.sh
```

Скрипт запросит:
- Yookassa Shop ID и Secret Key
- 3X-UI URL и Token
- Telegram Bot Token
- Admin Telegram ID
- Домен для SSL

### Ручная установка

1. Установите Docker и docker-compose
2. Скопируйте `.env.template` в `.env` и заполните переменные
3. Запустите сервисы: `docker-compose up -d`
4. Выполните миграции: `docker-compose exec backend alembic upgrade head`

## 🔧 Конфигурация

### Переменные окружения (.env)

```env
# Database
DATABASE_URL=postgresql://user:password@db:5432/vpn_bot

# JWT
JWT_SECRET=your_jwt_secret_here

# Admin credentials
ADMIN_LOGIN=admin
ADMIN_PASSWORD=123456

# 3X-UI Integration
XUI_URL=https://your-xui-server.com
XUI_TOKEN=your_xui_token_here

# Yookassa Integration
YOOKASSA_SHOP_ID=your_yookassa_shop_id
YOOKASSA_SECRET_KEY=your_yookassa_secret_key

# Telegram Bot
TELEGRAM_BOT_TOKEN=your_telegram_bot_token
ADMIN_TELEGRAM_ID=your_admin_telegram_id

# Domain
DOMAIN=your-domain.com
SSL_EMAIL=your-email@example.com
```

## 📡 API Endpoints

### Аутентификация
- `POST /auth/login` - Вход администратора

### Пользователи
- `GET /users` - Список пользователей
- `GET /users/{id}` - Информация о пользователе
- `PUT /users/{id}/extend` - Продлить подписку
- `DELETE /users/{id}` - Удалить пользователя

### Платежи
- `GET /payments` - История платежей
- `POST /payments/create` - Создать платеж

### Тарифы
- `GET /tariffs` - Список тарифов
- `POST /tariffs` - Создать тариф

### Статистика
- `GET /dashboard/stats` - Статистика дашборда
- `GET /logs` - Логи системы

### Webhooks
- `POST /payments/yookassa/webhook` - Webhook от Юкассы
- `POST /bot/webhook` - Webhook от Telegram

## 🤖 Использование бота

1. Запустите бота командой `/start`
2. Выберите "🛒 Купить подписку"
3. Выберите тариф
4. Оплатите через Юкассу
5. После оплаты доступы придут автоматически

### Команды администратора
- `/admin` - Панель администратора в боте

## 🔒 Безопасность

- JWT аутентификация для API
- HTTPS через Let's Encrypt
- Валидация всех входных данных
- Логирование всех операций
- Защита от brute-force

## 📊 Мониторинг

- Логи в базе данных
- Docker logs для каждого сервиса
- Метрики производительности

## 🚀 Деплой

Проект использует GitHub Actions для автоматического деплоя:

1. Настройте секреты в GitHub:
   - `SSH_PRIVATE_KEY` - SSH ключ для сервера
   - `SERVER_HOST` - IP сервера
   - `SERVER_USER` - Пользователь на сервере

2. Push в main ветку автоматически запустит деплой

## 📝 Разработка

### Backend
```bash
cd backend
pip install -r requirements.txt
uvicorn main:app --reload
```

### Frontend
```bash
cd frontend
npm install
npm run dev
```

### Bot
```bash
cd bot
pip install -r requirements.txt
python main.py
```

## 📄 Лицензия

MIT License

## 🤝 Поддержка

Для вопросов и поддержки:
- Создайте issue в репозитории
=======
# VPN Bot - Сервис продажи VPN-подписок

Полнофункциональный сервис для продажи VPN-подписок через 3X-UI с веб-панелью администратора, Telegram-ботом и автоматической установкой.

## 🚀 Возможности

### Веб-панель администратора
- 🔐 Авторизация с JWT
- 📊 Дашборд с статистикой (выручка, пользователи, платежи)
- 👥 Управление пользователями (просмотр, продление, отключение, удаление)
- 💸 История платежей с фильтрами
- 💳 Настройки тарифов
- ⚙️ Настройки интеграций (3X-UI, Юкасса, Telegram)
- 📦 Логи всех операций
- 🔧 DevOps функции (перезапуск сервисов, обновление)

### Telegram-бот
- 🛒 Покупка VPN-подписок
- 🔄 Продление подписки
- 📅 Проверка статуса
- 🔗 Получение конфигураций
- ⚙️ Личный кабинет
- 💰 Оплата через Юкассу

### Интеграции
- 🔗 3X-UI API для управления клиентами
- 💳 Юкасса для платежей
- 🤖 Telegram Bot API

## 🛠 Технологии

- **Backend**: Python FastAPI, PostgreSQL, SQLAlchemy
- **Frontend**: Next.js 14, React, Tailwind CSS, shadcn/ui
- **Bot**: Python aiogram 3
- **Инфраструктура**: Docker, docker-compose, nginx, certbot

## 📦 Установка

### Автоматическая установка (рекомендуется)

```bash
# Скачайте репозиторий
git clone https://github.com/your-repo/vpn-bot.git
cd vpn-bot

# Запустите скрипт установки
chmod +x install.sh
sudo ./install.sh
```

**Поддерживаемые ОС**: Ubuntu 22.04, Ubuntu 24.04

Скрипт запросит:
- Yookassa Shop ID и Secret Key
- 3X-UI URL и Token
- Telegram Bot Token
- Admin Telegram ID
- Домен для SSL

### Ручная установка

1. Установите Docker и docker-compose
2. Скопируйте `.env.template` в `.env` и заполните переменные
3. Запустите сервисы: `docker-compose up -d`
4. Выполните миграции: `docker-compose exec backend alembic upgrade head`

## 🔧 Конфигурация

### Переменные окружения (.env)

```env
# Database
DATABASE_URL=postgresql://user:password@db:5432/vpn_bot

# JWT
JWT_SECRET=your_jwt_secret_here

# Admin credentials
ADMIN_LOGIN=admin
ADMIN_PASSWORD=123456

# 3X-UI Integration
XUI_URL=https://your-xui-server.com
XUI_TOKEN=your_xui_token_here

# Yookassa Integration
YOOKASSA_SHOP_ID=your_yookassa_shop_id
YOOKASSA_SECRET_KEY=your_yookassa_secret_key

# Telegram Bot
TELEGRAM_BOT_TOKEN=your_telegram_bot_token
ADMIN_TELEGRAM_ID=your_admin_telegram_id

# Domain
DOMAIN=your-domain.com
SSL_EMAIL=your-email@example.com
```

## 📡 API Endpoints

### Аутентификация
- `POST /auth/login` - Вход администратора

### Пользователи
- `GET /users` - Список пользователей
- `GET /users/{id}` - Информация о пользователе
- `PUT /users/{id}/extend` - Продлить подписку
- `DELETE /users/{id}` - Удалить пользователя

### Платежи
- `GET /payments` - История платежей
- `POST /payments/create` - Создать платеж

### Тарифы
- `GET /tariffs` - Список тарифов
- `POST /tariffs` - Создать тариф

### Статистика
- `GET /dashboard/stats` - Статистика дашборда
- `GET /logs` - Логи системы

### Webhooks
- `POST /payments/yookassa/webhook` - Webhook от Юкассы
- `POST /bot/webhook` - Webhook от Telegram

## 🤖 Использование бота

1. Запустите бота командой `/start`
2. Выберите "🛒 Купить подписку"
3. Выберите тариф
4. Оплатите через Юкассу
5. После оплаты доступы придут автоматически

### Команды администратора
- `/admin` - Панель администратора в боте

## 🔒 Безопасность

- JWT аутентификация для API
- HTTPS через Let's Encrypt
- Валидация всех входных данных
- Логирование всех операций
- Защита от brute-force

## 📊 Мониторинг

- Логи в базе данных
- Docker logs для каждого сервиса
- Метрики производительности

## 🚀 Деплой

Проект использует GitHub Actions для автоматического деплоя:

1. Настройте секреты в GitHub:
   - `SSH_PRIVATE_KEY` - SSH ключ для сервера
   - `SERVER_HOST` - IP сервера
   - `SERVER_USER` - Пользователь на сервере

2. Push в main ветку автоматически запустит деплой

## 📝 Разработка

### Backend
```bash
cd backend
pip install -r requirements.txt
uvicorn main:app --reload
```

### Frontend
```bash
cd frontend
npm install
npm run dev
```

### Bot
```bash
cd bot
pip install -r requirements.txt
python main.py
```

## 📄 Лицензия

MIT License

## 🤝 Поддержка

Для вопросов и поддержки:
- Создайте issue в репозитории
>>>>>>> b57780b3d4d2f8339e1ca7e5946a11641bbc5f64
- Напишите в Telegram: @your_support_bot